...
fn it_omits_target_list() {
    ...
    let result = summary(query, 0, 40).unwrap();
...
fn it_omits_WHERE_clause() {
    ...
    let result = summary(query, 0, 30).unwrap();
...
fn it_omits_INSERT_field_list() {
    ...
    let result = summary(query, 0, 32).unwrap();
    assert_eq!(result.truncated_query, "INSERT INTO x (...) VALUES (...)")
}

#[test]
fn it_omits_comments() {
    let query = "SELECT $1 /* application:test */";
    let result = summary(query, 0, 100).unwrap();
...
fn it_falls_back_to_simple_truncation() {
    ...
    let result = summary(query, 0, 10).unwrap();
...
fn it_handles_problematic_cases() {
    ...
    let result = summary(query, 0, 50).unwrap();
    assert_eq!(result.truncated_query, "SELECT ...")
}
...
fn it_omits_ON_CONFLICT_target_list() {
    ...
    let result = summary(query, 0, 64).unwrap();
...
fn it_handles_GRANT() {
    ...
    let result = summary(query, 0, 35).unwrap();
...
fn it_handles_CREATE_INDEX() {
    ...
    let result = summary(query, 0, 60).unwrap();
...
fn char_truncate_works() {
    ...
    let result = summary(query, 0, 21).unwrap();
